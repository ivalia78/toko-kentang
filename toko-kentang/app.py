...
DATA_FILE = os.path.join("data", "keuangan_kentang_streamlit.json")
...
def load_data():
    if os.path.exists(DATA_FILE):
        with open(DATA_FILE, "r") as f:
            data = json.load(f)
        # Pastikan kunci ada supaya tidak error
        if "jurnal_umum" not in data: data["jurnal_umum"] = []
        ...
        return {"jurnal_umum": [], "next_id": 1, "users": [], "orders": [], "feedbacks": []}
...
def save_data(data):
    ...
    # Sinkron juga session_state untuk kemudahan akses
